...
import (
	...
	raudio "game/internal/resources/audio"
	rfonts "game/internal/resources/fonts"
	rimages "game/internal/resources/images"
)
...
const (
	// ScreenWidth defines the width of the screen.
	ScreenWidth = 640
	// ScreenHeight defines the height of the screen.
	ScreenHeight     = 480
	tileSize         = 32
...
func init() {
	img, _, err := image.Decode(bytes.NewReader(rimages.Gopher))
	if err != nil {
		log.Fatal(err)
	}
	gopherImage = ebiten.NewImageFromImage(img)

	img, _, err = image.Decode(bytes.NewReader(rimages.Tiles))
	if err != nil {
		log.Fatal(err)
	}
...
func init() {
	tt, err := opentype.Parse(rfonts.PressStart2P)
	if err != nil {
		log.Fatal(err)
	}
	const dpi = 72
...
// Mode is the current state of the game.
type Mode int

const (
	// ModeTitle is the state when the game is in the title screen.
	ModeTitle Mode = iota
	// ModeGame is the state when the game is ongoing.
	ModeGame
	// ModeGameOver is the state when the game is over.
	ModeGameOver
)

// Game is the struct containing the game state.
type Game struct {
	mode Mode
...
func (g *Game) init() {
	...
	for i := range g.pipeTileYs {
		...
		g.pipeTileYs[i] = int(randInt.Int64() + 2)
	}
	...
	jumpD, err := vorbis.DecodeWithoutResampling(bytes.NewReader(raudio.Jump))
	if err != nil {
		log.Fatal(err)
	}
	...
	jabD, err := wav.DecodeWithoutResampling(bytes.NewReader(raudio.Jab))
	if err != nil {
		log.Fatal(err)
	}
...
func (g *Game) drawTiles(screen *ebiten.Image) {
	...
	for i := -2; i < nx+1; i++ {
		...
		subimage, _ := tilesImage.SubImage(image.Rect(0, 0, tileSize, tileSize)).(*ebiten.Image)
		...
		if tileY, ok := g.pipeAt(floorDiv(g.cameraX, tileSize) + i); ok {
			for j := 0; j < tileY; j++ {
				...
				subimage, _ := tilesImage.SubImage(r).(*ebiten.Image)
				screen.DrawImage(subimage, op)
			}
